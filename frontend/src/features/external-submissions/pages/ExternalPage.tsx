...
export function ExternalPage() {
  ...
  const today = useQuery({ queryKey: ['today'], queryFn: () => getJson<DailyAssignment>('/assignments/today'), enabled: Boolean(roadmap.data) && !roadmap.data?.goalAchievedAt });
  ...
  const submitResult = useMutation({
    ...
    onSuccess: () => Promise.all([queryClient.invalidateQueries({ queryKey: ['today'] }), queryClient.invalidateQueries({ queryKey: ['external-history'] }), queryClient.invalidateQueries({ queryKey: ['profile'] }), queryClient.invalidateQueries({ queryKey: ['roadmap'] }), queryClient.invalidateQueries({ queryKey: ['weekly-report'] })]),
  });
