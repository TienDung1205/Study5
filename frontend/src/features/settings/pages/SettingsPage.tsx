...
export function SettingsPage() {
  ...
  useEffect(() => { const goal = profile.data?.learningGoal; if (!goal) return; setDailyMinutes(goal.dailyMinutes); setPreferredHour(goal.preferredHour); setExamDate(goal.examDate?.slice(0, 10) ?? ''); setStudyDays(goal.studyDays); }, [profile.data]);
  const save = useMutation({ mutationFn: () => putJson('/users/me/learning-goal', { dailyMinutes, preferredHour, examDate: examDate || undefined, studyDays }), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['profile'] }) });
  ...
  return <section><header className="page-header"><div><p className="eyebrow">CÁ NHÂN HÓA</p><h2>Mục tiêu học tập</h2><p className="muted">Lịch học mặc định từ thứ Hai đến thứ Bảy.</p></div></header>
    <div className="goal-score-summary"><span>Điểm checkpoint hiện tại<strong>{profile.data?.learningGoal?.currentScore ?? 'Chưa có'}</strong></span><span>Mục tiêu đang học<strong>{profile.data?.learningGoal?.targetScore ?? 'Chưa có'}</strong></span></div>
    <form className="settings-form" onSubmit={submit}>
      ...
      <button className="primary-button" disabled={save.isPending || studyDays.length !== 6}>Lưu thay đổi</button></form>
  </section>;
}
