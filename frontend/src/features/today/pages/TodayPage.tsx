...
export function TodayPage() {
  ...
  const accessibleLessons = roadmap.data ? getAccessibleLessons(roadmap.data, user?.role === 'ADMIN') : [];
  const currentAssignedLessonId = today.data?.items.find((item) => item.lesson && !item.completedAt)?.lesson?.id;
  const nextPlannedLessonId = today.data?.status === 'EXCUSED'
    ? undefined
    : roadmap.data?.phases
      .filter((phase) => !phase.skipped)
      .flatMap((phase) => phase.lessons)
      .find((lesson) => !lesson.completed && lesson.unlocked)?.id;
  const continueLessonId = currentAssignedLessonId ?? nextPlannedLessonId;
  useEffect(() => {
    if (user?.role === 'LEARNER' && !requestedLessonId && continueLessonId) {
      setSearchParams({ lesson: continueLessonId }, { replace: true });
    }
  }, [continueLessonId, requestedLessonId, setSearchParams, user?.role]);
  ...
  return <section className="today-learning-page">
    ...
    {requestedLessonId && !requestedLesson && <ToastMessage variant="warning">Bạn chưa được mở bài này trong lộ trình.</ToastMessage>}
    ...
    {selectedLesson ? <div className="today-lesson-shell"><LessonContent lesson={selectedLesson} assignment={today.data} dailyItem={selectedAssignmentItem} targetScore={roadmap.data.targetScore} /></div> : <div className="empty-state">Chưa có bài học để hiển thị.</div>}
...
function PageHeading({ admin }: { admin: boolean }) {
  return <header className="page-header today-header"><div><Link className="back-to-roadmap" to="/roadmap"><ArrowLeft size={16} /> Quay lại lộ trình</Link><p className="eyebrow">{admin ? 'CHẾ ĐỘ QUẢN TRỊ · XEM TOÀN BỘ' : 'BÀI TIẾP THEO TRONG LỘ TRÌNH'}</p><h2>{admin ? 'Xem nội dung bài học' : 'Tiếp tục học'}</h2><p className="muted">{admin ? 'Chọn bất kỳ bài nào từ Lộ trình để kiểm tra nội dung.' : 'Tự động mở đúng bài bạn cần hoàn thành tiếp theo.'}</p></div></header>;
}
